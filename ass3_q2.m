...
load occmap.mat; %load occmap_work.mat; %for changes 
...
laser_var = 0.3^2;      % variance on laser range distribution
w_gain = 10*sqrt( 2 * pi * laser_var );     % gain on particle weight

% generate an initial cloud of particles, initialization
...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            %Check if scans are NaN and time stamp interpolation errors and
            %r_min_laser and y_laser_max
            if omega_max > abs(omega_interp(i)) && ~isnan(y_laser(i,j)) && y_laser(i,j) > r_min_laser && y_laser(i,j) < y_laser_max
            
                %Get laser scan distance
                scan_dist = y_laser(i,j);
            
                %particle coord laser scan origin 
                x_particle_coord = round((x_particle(n) + d_scan*cos(theta_particle(n)) - ogxmin)/ogres);
                y_particle_coord = round((y_particle(n) + d_scan*sin(theta_particle(n)) - ogymin)/ogres);
                theta_particle_scan = theta_particle(n) + angles(j); %particle coord laser scan angle
                
                %initiallize particle distance to obstacle
                particle_dist = nan;
                
                %loop over laser distances, reverse raytracing
                for l = r_min_laser:sqrt(2)*ogres:y_laser_max
                    
                    %find coordinates of scan with variable distance
                    x_scan_coord = x_particle_coord + round(l*cos(theta_particle_scan)/ogres);
                    y_scan_coord = y_particle_coord + round(l*sin(theta_particle_scan)/ogres);
                    
                    %check if coordinates are within occupancy map
                    if x_scan_coord > 300 || y_scan_coord > 180 || x_scan_coord < 1 || y_scan_coord < 1
                        break %exit loop
                    end
                    
                    %check if log odds have obstacle
                    if oglo(y_scan_coord, x_scan_coord) > 0
                       particle_dist = l; %set particle distance to obstacle
                       break %exit loop
                    end
                end
                
                %check if particle distance exists
                if isnan(particle_dist)
                    continue
                end
                
                %calculate difference between expected and particle scan
                error = (particle_dist - scan_dist);
                 
                %Use Guassian PDF, stddev^2 = var
                p_gaussian = (1/sqrt(2*pi*laser_var))*exp(-error^2/(2*laser_var));
                
                %Update particle weight using gain and PDF
                w_particle(n) = w_particle(n) + w_gain*p_gaussian;
            end
    ...
    %2. resample posterior based on the weight assigned, done below
    % resample the particles using Madow systematic resampling
    w_bounds = cumsum(w_particle)/sum(w_particle); %Bm
    w_target = rand(1);
    j = 1;
    for n=1:nparticles
    ...
    x_particle = x_particle_new; %assign new particle coord based on weights
